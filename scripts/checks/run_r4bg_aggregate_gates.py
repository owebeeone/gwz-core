...
while printing the wrong count still fails here. This DRIVES the batteries and
re-implements none of them; every command is one the runbook already names.
...
A full pass is ~25 min, dominated by the call-graph compiler probes and the
release-profile root fault matrix; `battery:index` exists so a host with a
per-command time budget can partition it the way `GwzM5-8R4bG-Evidence.md`
§3.1 partitions the lib suite. A partitioned run prints PARTIAL and withholds
the aggregate pass line, so reconciling across invocations stays the reader's
job, explicitly. Commands run in gwz-core whatever the caller's cwd.
"""
...
BATTERIES: dict[str, tuple[str, list[tuple[str, list[str], str]]]] = {
    ...
    "byte-equivalence": ("byte-equivalence gate, both halves of O8 (rows 2.3a/2.3b, §12)", [
        ("M4 scenario map", check("check_m4_scenario_map.py"), "M4 scenario map: ok"),
