...
# The Option A form (LCM1.0c follow-up 2): `cargo test -p <name> --lib
# --locked` from gwz-core over the inventory's present packages, exactly as
# `.github/workflows/checked-artifact-boundary.yml` runs it, the command
# wrapped across `\` continuations so the shape guard is exercised on the
# new form too.
PACKAGE_LOCKED_WORKFLOW = """\
...
jobs:
  libraries:
    runs-on: ubuntu-24.04
    steps:
      - name: Each library's Tier A command, locked against the workspace lock
        run: |
          # cargo test -p gwz-alpha --lib
          names="$(python scripts/checks/check_local_clone_boundaries.py --list-present)"
...
PACKAGE_UNLOCKED_WORKFLOW = PACKAGE_LOCKED_WORKFLOW.replace(
    "              --lib --locked\n", "              --lib\n"
)

CORE_LOCK_TEXT = "# This file is automatically @generated by Cargo.\nversion = 4\n"
...
class SyntheticTree:
    ...
    def write_core_manifest(
        ...
        *,
        ...
        """The gwz-core manifest of layout Option A: the package, the
        `[workspace]` whose members are the crates (`crates/*` unless
        `members` lists them), and the committed workspace lock."""
        deps = "".join(
        ...
        workspace_table = ""
        if workspace:
            listed = ", ".join(f'"{member}"' for member in (members or ["crates/*"]))
            workspace_table = f'[workspace]\nresolver = "2"\nmembers = [{listed}]\n'
        ...
        lock_path = self.core / "Cargo.lock"
        if lock:
            lock_path.write_text(CORE_LOCK_TEXT, encoding="utf-8")
    ...
    def check(self, **core_manifest: object) -> subprocess.CompletedProcess[str]:
        self.write_core_manifest(**core_manifest)
...
class LocalCloneBoundaryTest(unittest.TestCase):
    ...
    def test_declared_third_party_with_unlocked_tier_a_is_rejected(self) -> None:
        ...
        # tree with a locked Tier A step passes. LCM1.0c follow-up 2 retired
        # the guard on its recorded condition (Option A landed, `--locked`
        # restored): the workflows are now the only evidence, in the
        # `cargo test -p <name> --lib --locked` form, and the former
        # `ci_tier_a_unlocked` inventory flag is ignored either way.
        gate = load_gate()
        tree = SyntheticTree()
        self.addCleanup(tree.cleanup)
        tree.third_party_alpha()
        tree.workflow("boundary.yml", PACKAGE_UNLOCKED_WORKFLOW)
        ...
        # The retired flag moves nothing in either direction.
        tree.inventory["ci_tier_a_unlocked"] = True
        self.assertFalse(gate.tier_a_unlocked(tree.core, tree.inventory))
        self.assertEqual(tree.check().returncode, 0)
        tree.workflow("boundary.yml", PACKAGE_UNLOCKED_WORKFLOW)
        tree.inventory["ci_tier_a_unlocked"] = False
        self.assertTrue(gate.tier_a_unlocked(tree.core, tree.inventory))
        self.assertNotEqual(tree.check().returncode, 0)

    def test_core_manifest_is_the_workspace_and_carries_its_lock(self) -> None:
        # Layout Option A (LCM1.0c follow-up 2): the gwz-core manifest declares
        # the `[workspace]` whose members are the crates, and the workspace
        # lock is committed beside it. A member listed explicitly is as good
        # as the glob; an unlisted crate, a missing table or a missing lock
        # are findings.
        tree = SyntheticTree()
        self.addCleanup(tree.cleanup)
        result = tree.check(members=["crates/alpha-contract", "crates/alpha"])
        self.assertEqual(result.returncode, 0, result.stderr + result.stdout)

        result = tree.check(members=["crates/alpha-contract"])
        self.assertNotEqual(result.returncode, 0, result.stdout)
        self.assertIn("gwz-alpha: not a member of gwz-core's `[workspace]`", result.stderr)

        result = tree.check(workspace=False)
        self.assertNotEqual(result.returncode, 0, result.stdout)
        self.assertIn("must declare the `[workspace]`", result.stderr)

        result = tree.check(lock=False)
        self.assertNotEqual(result.returncode, 0, result.stdout)
        self.assertIn("workspace lock is missing", result.stderr)

    def test_real_tier_a_step_is_locked_and_lists_the_present_packages(self) -> None:
        ...
        listed = subprocess.run(
            [sys.executable, str(SCRIPT), "--list-present"],
            check=False,
            capture_output=True,
            text=True,
        )
