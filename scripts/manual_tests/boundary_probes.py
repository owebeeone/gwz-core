...
SCRIPT = Path(__file__).resolve().parents[1] / "checks/check_checked_artifact_boundaries.py"
...
class CheckedArtifactBoundaryTest(unittest.TestCase):
    ...
    def test_preservation_plan_caller_is_inside_the_source_allowlist(self) -> None:
        ...
        path.write_text(
            path.read_text(encoding="utf-8").replace(
                "backend.preservation_stashes(&plan.path, &record.merge_id)",
    ...
    def test_checked_bundle_observer_uses_shared_repository_contract(self) -> None:
        checked_bundle = (SOURCE / "workspace_ops/merge/preserve/checked_bundle.rs").read_text(encoding="utf-8")
        contract = (SOURCE / "git/gitbackend/contract.rs").read_text(encoding="utf-8")
        self.assertIn("backend.preservation_stashes", checked_bundle)
        self.assertIn("fn preservation_stashes(", contract)
        self.assertIn("MergeAuthorityBackend", checked_bundle)
