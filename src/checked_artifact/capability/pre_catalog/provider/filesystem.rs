...
use super::{
    ...
    RawPreCatalogProviderV1,
    digests::{ReadyDigestInputsV1, derive_missing_digest, derive_ready_digests},
    index, namespace, retained, snapshot,
};
...
mod bound;
pub(in crate::checked_artifact::capability::pre_catalog) use bound::{
...
struct Observed {
    ...
    ready_digests: Option<super::ReadyObservationDigestsV1>,
    missing_parent_digest: Option<MissingParentObservationDigestV1>,
...
impl<P: PlatformProviderV1 + 'static> FilesystemPreCatalogProvider<P> {
    ...
    fn finish(
        ...
        let collision_snapshot_digest = snapshot::digest(SnapshotParts {
            ...
            namespace_entry_count: namespace.enumeration.entry_count(),
            namespace_encoded_name_bytes: namespace.enumeration.encoded_name_bytes(),
        });
        let ready_digests = private_parent_fact
            .as_deref()
            .map(|private_parent_fact| {
                derive_ready_digests(ReadyDigestInputsV1 {
                    root_kind,
            ...
            .transpose()?;
        let missing_parent_digest = if private_parent_fact.is_none() {
            Some(derive_missing_digest(
                root_kind,
                self.platform.support_profile(),
                &retained,
                &path_profile,
            ))
        } else {
    ...
    fn compare(
        ...
        if observed.support_profile != expected.support_profile
            ...
            || observed.ready_digests != expected.ready_digests
            || observed.missing_parent_digest != expected.missing_parent_digest
...
impl Observed {
    fn into_raw(self) -> RawPreCatalogObservationV1<RetainedPlatformRoot> {
        RawPreCatalogObservationV1 {
            ...
            ready_digests: self.ready_digests,
