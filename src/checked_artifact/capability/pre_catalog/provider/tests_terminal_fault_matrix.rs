...
use crate::checked_artifact::protocol::{
    ...
    DurableLeafFingerprintV1, InfrastructureSlotV1, ManagedBootstrapInputV1, RecordObservationV1,
    RequestOwnerBindingV1, RootEntryNameV1, admit_observed_action,
};
...
impl Fixture {
    ...
    fn admit(&self, expected: &ActionCapacityReservationV1) -> DurableObjectIdentityV1 {
        let identity = with_catalog(self, |catalog| {
...
/// cleanup row complete, i.e. the live row gone and the retired alias resident.
fn place_completed_action_rows(action_directory: &Path, expected: &ActionCapacityReservationV1) {
    ...
    for slot in [
        ...
        fs::write(
            action_directory.join(slot_name(slot, action)),
            b"gwz-retired-alias-fixture\n",
        )
        .expect("the admitted action directory is writable");
    }
    let identity = DurableObjectIdentityV1::linux_ext4([7; 16], 1, vec![9; 8])
        .expect("the fixture identity is well formed");
    let rows = CleanupAliasV1::ALL
        .into_iter()
        .map(|alias| {
            CleanupRowV1::new(
                alias,
                DurableLeafFingerprintV1::new(identity.clone(), 26, [5; 32]),
            )
        })
        .collect::<Vec<_>>();
...
/// Rebuilds the handoff for a restart.
///
/// Same reason `namespace/tests_fault_matrix.rs:25-34` gives, inherited rather
/// than re-owned: once any row is resident the action directory is no longer
/// *exact*, which is precisely the state a second admission must refuse, and
/// resuming the handoff from durable state is **item 6 of the Phase 3 settle
/// docket** — not owned by any landed step. `retain_action_namespace` still
/// fails closed if the reconstructed identity is not the resident one, which is
/// what makes inheriting it safe rather than merely convenient.
fn handoff(
...
fn attempt(
    ...
    with_catalog(fixture, |catalog| {
        catalog.retire_admitted_action(&handoff(expected, identity))
    })
}
...
fn settle(
    ...
    attempt(fixture, expected, identity)
...
fn run_interruption_matrix(variant: TargetVariantV1) {
    ...
    let settled = {
        ...
        let identity = fixture.admit(&expected);
        let settled = settle(&fixture, &expected, &identity, "baseline");
    ...
    for key in TERMINAL_MATRIX {
        ...
        let interrupted = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let _ = attempt(&fixture, &expected, &identity);
        }));
        assert!(
            interrupted.is_err(),
        ...
        let resumed = settle(&fixture, &expected, &identity, &stable);
        assert_eq!(
            resumed, settled,
        ...
        let again = settle(&fixture, &expected, &identity, &stable);
        assert_eq!(
            again, settled,
...
fn run_repeated_boundary_crashes(variant: TargetVariantV1) {
    ...
    let settled = {
        ...
        let identity = fixture.admit(&expected);
        settle(&fixture, &expected, &identity, "baseline")
    };
    ...
    for key in REPEATED_BOUNDARIES {
        ...
        let converged = settle(&fixture, &expected, &identity, &stable);
        assert_eq!(
            converged, settled,
...
fn the_retired_child_keeps_the_derived_active_action_name() {
    ...
    let identity = fixture.admit(&expected);
    settle(&fixture, &expected, &identity, "decision-t-a");
...
fn a_retired_root_holding_an_infrastructure_row_refuses_the_retirement() {
    ...
    let refusal = attempt(&fixture, &expected, &identity);
    assert!(
        refusal.is_err(),
