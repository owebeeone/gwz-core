...
// The retained filesystem adapter shares DELETE access on Windows. Detect
// substitution through identity revalidation and preserve the foreign tree.
#[cfg(windows)]
#[test]
fn retained_directory_rejects_substitution_windows() {
    let root = TempRoot::new("retained-parent-detects-substitution");
    fs::create_dir_all(root.0.join("a")).unwrap();
    fs::write(root.0.join("a/value"), b"owned").unwrap();
    let checked = artifact(&root.0, "a/value");
    fs::rename(root.0.join("a"), root.0.join("old-a")).unwrap();
