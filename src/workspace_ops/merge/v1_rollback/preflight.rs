        ...
        for relative in [WORKSPACE_MANIFEST, artifact::LOCK_PATH] {
            if !matches!(
                ...
                return Err(ModelError::new(
                    ...
                    format!("selected-root artifact '{relative}' is not a canonical regular file"),
                ));
            }
        }
    }
...
fn require_virtual_selected_root_after_evidence<B: GitBackend>(
    ...
    let publication = record
        .publication
        .as_ref()
        .ok_or_else(|| member_preflight_error("@root", ".", "publication evidence disappeared"))?;
    let candidate = publication
        .candidate
        .as_ref()
        .ok_or_else(|| member_preflight_error("@root", ".", "publication candidate disappeared"))?;
