...
pub(crate) fn init_from_sources_can_create_workspace_clone_local_urls_and_write_lock() {
    ...
    assert_eq!(
        root_status.untracked, 0,
        "the member is excluded, not untracked"
    );
    assert!(
        root_status
            .files
...
pub(crate) fn init_rolls_back_fresh_clones_on_mid_batch_failure() {
    ...
    assert!(
        result.is_err(),
        "init must fail when a source fails to clone"
    );
