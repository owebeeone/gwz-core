...
pub(super) fn source_marker_is_committed(root: &Path) -> bool {
    fn check(root: &Path) -> Option<bool> {
        ...
        if indexed.id != committed.id()
            || indexed.mode != 0o100644
            || committed.filemode() != 0o100644
            || indexed.flags & 0x8000 != 0
...
pub(super) fn discount_generated_marker(
    root: &Path,
    ...
    if !known.unknown.is_empty()
        || !known.suppressed.is_empty()
        || !known.sparse_absent.is_empty()
        || known.native_operation.is_some()
        || known
            .entries
            .iter()
            .any(|entry| entry.path.as_slice() == marker && entry.kind != WorkKind::Unstaged)
    {
        return None;
    }
    if !known
        .entries
        .iter()
        .any(|entry| entry.path.as_slice() == marker)
    {
        return None;
    }
    ...
    known
        .entries
        .retain(|entry| entry.path.as_slice() != marker);
    Some(head)
}
...
pub(super) fn admitted_marker_head(root: &Path) -> Option<String> {
    ...
    for relative in GUARDED_CONF_PATHS
        .into_iter()
        .chain([CONF_INTEGRITY_MARKER_PATH])
    {
        ...
        if !fs::symlink_metadata(&physical).ok()?.is_file() {
            return None;
        }
        let indexed = index.get_path(path, 0)?;
        let committed = tree.get_path(path).ok()?;
        if indexed.id != committed.id()
            || indexed.mode != committed.filemode() as u32
            || indexed.mode != 0o100644
            || indexed.flags & 0x8000 != 0
            || indexed.flags_extended != 0
        {
        ...
        if relative != CONF_INTEGRITY_MARKER_PATH {
            let blob = repo.find_blob(indexed.id).ok()?;
            if fs::read(&physical).ok()?.as_slice() != blob.content() {
                return None;
            }
        }
    }
    let canonical = canonical_conf_integrity_marker(root).ok()??;
    (fs::read(root.join(CONF_INTEGRITY_MARKER_PATH)).ok()? == canonical.as_bytes())
        .then(|| head.id().to_string())
}
