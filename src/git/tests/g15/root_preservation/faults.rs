...
fn granular_leaf_and_index_faults_have_exact_restart_classification() {
    let cases = [
        (
            1,
            FaultBoundary::BeforeLeafUnlink,
            GitRootPreservationStepObservation::Before,
        ),
        (
            1,
            FaultBoundary::AfterLeafUnlink,
            GitRootPreservationStepObservation::After,
        ),
        (
            2,
            FaultBoundary::BeforeLeafRename,
            GitRootPreservationStepObservation::Before,
        ),
        (
            2,
            FaultBoundary::AfterLeafRename,
            GitRootPreservationStepObservation::After,
        ),
        (
            3,
    ...
    for (step_index, boundary, expected) in cases {
        let fixture = fixture();
        let guard = guard(&prepare(&fixture));
