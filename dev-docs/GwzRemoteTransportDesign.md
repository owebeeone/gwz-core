...
All Git network work uses a transport endpoint through one taut message API.
...
|---|---|---|
| `Open` | Initiator → endpoint | Operation id; captured destination and Git service; identity selection and endpoint-local path base; deadlines; caller's receive-window size. Requests a lease and exchange. |
...
| `Close` | Initiator → endpoint | Graceful release request after locally buffered writes have been sent; endpoint finishes bounded cleanup. |
| `Closed` | Endpoint → initiator | Terminal exchange result, reusable/discarded disposition and final transport facts. Not a substitute for Git's result. |
...
Open's transport descriptor contains the effective URL and a typed Git service
...
| Active/half-closed | Graceful `Close` | Flush local pending writes, enter closing, finish backend cleanup |
...
Transport registration is process-wide in the pinned git2 binding and must be
synchronized during runtime initialization. Per-operation route and identity
state must not live in a mutable global "current operation". Proposed binding:
a scoped thread-local context around each synchronous libgit2 network call;
the registered factory captures an owned context into the subtransport, then
all stream work uses that captured context. Guards restore previous context
on success, error and unwinding. Unknown/unbound factory calls fail rather than
inheriting some other operation's endpoint. Qualify synchronous factory/thread
behaviour, nesting and concurrent identities before adopting this mechanism;
if unsupported, use a per-remote callback binding rather than URL-based global
lookup. Document the process-wide registration impact for library embedders.
...
Extend taut capabilities additively with the supported message protocol version,
...
| Lifecycle | Half-close retains reverse traffic; clean close versus failure; final-handle drop; duplicate terminal events and allocation/cancel races release once |
...
| Context isolation | Concurrent operations against the same URL with different endpoints/identities; nested native calls; error/unwind cleanup of factory context |
| Compatibility/reporting | Old/new capability combinations, missing fields, explicit-route refusal, offered/authenticated/reused facts in ordinary, streaming and error responses |
...
| Native context binding | Scoped synchronous factory capture | Concurrent/nested-call isolation demonstrated; replace binding if the pinned API cannot support it |
...
- Pinned local git2 0.21.0, `src/transport.rs`: `SmartSubtransport`,
  `Transport::smart`, process-wide `register`, stateful stream continuation and
  `stream_write` calling `write_all`. These are version-specific integration
  facts to recheck if the dependency changes.
